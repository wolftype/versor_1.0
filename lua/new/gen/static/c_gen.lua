...
package.path = "../?.lua;../../vsrmodule/?.lua;../../util/?.lua;" .. package.path
...
local path = cwd.."codegen/VSR/"--env.."/code/versor/branches/subspace/lua/staticCGA/gen/vsr/include/"
local cpath = cwd.."codegen/src/"--env.."/code/versor/branches/subspace/lua/staticCGA/gen/vsr/src/"
...
local productGenTemplated = function(tx, ty, top)
	...
	local list = F.productList(tx, ty, top, AllStates)
	local typ =  list.type
		...
		local code = cosmo.f ( template ) {	
			make_function = function()
				...
				cosmo.yield{
					...
					lhand = tx.id,
					rhand = ty.id,
					rcombo = T.makePType(list.inst, "a", "b")
				}
...
local versionGen = function(tx, op)
	...
	local list = F.versionList(tx, op)
	local typ =  tx
	
	--why are we checking for table?
...
local versorGen = function(tx, ty, op, ct2)
	...
	local fun = op .. "(" .. tx.id .. " abc, " .. ty.id .. "ver) return"
	local exp = ""
	if op == "sp" then exp = tx.id .. "(ver * abc * !ver)" end
	if op == "sp0" then exp = " ver * abc * !ver " end
	if op == "sp1" then exp = ct2.id .. "(ver * abc * !ver)" end
	if op == "re" then exp = tx.id .. "(ver * #abc * !ver)" end
	if op == "re0" then exp = "ver * #abc * !ver" end
	
	local rt = Gen.func( fun..exp)
		...
		do_product = function()
			...
			--	print (iv.id)
				for k,kv in ipairs(GPTypes) do
					local glist = F.productList(iv, kv, "gp", AllStates)
					local gtyp =  glist.type
					local olist = F.productList(iv, kv, "op", AllStates)
					local otyp =  olist.type
					local ilist = F.productList(iv, kv, "ip", AllStates)
					local ityp =  ilist.type
