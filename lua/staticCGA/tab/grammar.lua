	...
	if op[1] == "gp" then 
		...
		for i, iv in ipairs (c) do
			...
			for j, jv in pairs(token) do
				out = out .. makePToken(jv, "t"..op[4], "t"..op[5] ) 
...
local assemble = function(tmp)
	...
	for k, kv in ipairs(tmp) do
		...
		if  nextop[1] == "cast" then
			...
			if kv[1] == "arg" then 
				var = makeAType( kv[2], kv[2].id:lower()  , "t"..k ) 
			end
			...
			if (kv[1] == "gp") or (kv[1] == "op") or (kv[1] == "ip") then 
				...
				head = " t"..k.." "
				var = makePType( pl, "t"..(kv[4]), "t"..(kv[5]) )..";" 
			end
...
--print ( getExp("Rot * Vec * !Rot") )
--print (makeExp("(Pnt ^ Pnt ^ Pnt ^ Inf) * !Pss"))
--print( makeExp("( ( (Pnt ^ Pnt ^ Pnt ^ Inf) * !Pss  ) ^ Pnt ) * !Pss") )
--print( makeExp("Dlp * Pnt") )

--print( getExp("Dlp ^ Pnt") )

--print( makeExp("Rot * Vec") )
--print( makeExp("Vec * Vec") )
...
local function eval(x)
	...
	if type(x) == "string" then
		...
		--if a table, then calculate lh op rh
		--if first element of table is special operator, do version conjugation . . .
		if ( vop:match(x[1]) ) then 
			print ("found", x[1])
			local lh = eval(x[2])
			return lh * -1
			...
			for i = 2, #x, 2 do
				...
				print ("lh: ", lh, op, "rh: ", rh)


				if (op == "^") 		then lh = lh + rh
				elseif (op == "-") 	then  lh = lh - rh
				elseif (op == "*") 	then  lh = lh * rh
				elseif (op == "<")  then  lh = lh/rh
				end
				print("nlh: ",lh)
