...
//			}
			...
			template < class S > 
			static Vec limit( const Lattice< S > & f, const State& p);
...
//			void drawVoxelLocAllSides(Vxl, int style = 1, bool grid=1); ///< Draw Voxel for round data elements (using Ro::cen) all Sides
	...
	void Lattice < T > :: advect( Lattice < S > & v, double dt, bool bounded, bool neg ) {
		...
		MonoFunc func;
		switch( v.type() ) {
			case _VEC:	func = Gen::trs;
				break;
			case _BIV:	func = Gen::rot;
				break;
			case _DLL:	func = Gen::mot_dll;
				break;
			case _TNV:	func = Gen::trv;
				break;
			case _MNK:	func = Gen::dil;
				break;
			case _DRV:	func = Gen::trs;
				break;
		}
