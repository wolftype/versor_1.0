...
struct ShaderParam {
    ...
    ShaderParam(ShaderParam::Type t, GLuint p, GLuint i = 0) : id(i), buf(256) { 
...
//    }
    ...
    void getAttrib(GLuint program) {
        ...
        type = GL::type(vectype); //Get GL Type from Vectype
    }

    void getUniform(GLuint program) {
        glGetActiveUniform(program, id, buf, &len, &size, &vectype, name);
        type = GL::type(vectype);
        
    }
    
    void print(){
        
        printf(" name: %s\n id: %d\n buf: %d\n size: %d\n length: %d\n bytes: %d\n", name, id, buf, size, len, GL::bpp(type));
