...
void basic(GLVApp& app){
    ...
    SET
        ...
        gui(num,"num",1,100)(amt,"amt")(bSkin, "skin");
...
void circularDini(GLVApp& app){
    ...
    ITI(i,num)
        Dll dll = ( Ta::at(c, p, false) ^ Inf(1) ).dual();
        Twist tw;
...
//    static Cir c = CYZ(1);
    ...
    ITI(i,iter)
        tp = Interp::linear<Par>(pa, pb, t);
        Pnt_Pnt pp = Gen::tpar( tp * amt);
        pp[0] = val;
        
        Dll dll = f1.zld().mot( Gen::dll_ratio( f1.zld(), f2.zld() ) * t );
        if (bDrawLines) dll.draw();
       ...
       ITI(j, iter)
            double xt = -1.0 + 2.0 * t;
            Pnt nsrc = ( bLine ? src2 : src ).trs( Fl::dir(dll, true) * xt * yamt).null();
            Pnt nnsrc = Ro::null_cen( nsrc.sp( pp ) );
            uvmesh.add(nnsrc);
            
            if (bShowBend){
                ITJ(k,num) VALJ(k,num)
                    Pnt tnsrc = Ro::null_cen( nsrc.sp( Gen::tpar( tp * amt * t ) ) );
                    tnsrc.draw(t,0,1-t);
                END 
            }
