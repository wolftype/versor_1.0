...
namespace vsr {
	...
	class Twist{
        ...
        Twist(const State& axis) : mExt(1) {
			...
			void moveBy( const State& vec ){
				mDll = Op::sp0( mDll , Gen::trs(vec) );
				//dll(td);
			}
			...
			Dll& dll() { return mDll; }
			Mot mot() const { return Gen::mot_dll( mDll ); }
			...
			/// Get Motor at t
			Mot mot( double t ) const { return Gen::mot_dll( mDll * t * mExt ); }
			
			/// Set Twist Axis From Destination Motor
			void set( const Mot& m){ mDll = Gen::log_mot(m); }
	...
	class CoupledTwist{	
		...
		public:
			...
			Dll dll(double t) { Mot m = mot(t); m/=m.rnorm(); return Gen::log_mot( m ); }
