...
Sca ip (Par a, Par b) { 
	return Sca (
                - a.e12 * b.e12 - a.e13 * b.e13 - a.e23 * b.e23 + a.e1no * b.e1ni + a.e2no * b.e2ni + a.e3no * b.e3ni + a.e1ni * b.e1no + a.e2ni * b.e2no + a.e3ni * b.e3no + a.noni * b.noni
                );
...
Pnt dls( float x, float y, float z, float sr, bool real) {
	Pnt s = null(x,y,z);
	if (real) s.ni -= .5 * (sr * sr)
    else s.ni += .5 * (sr*sr);
	return s;
}

Pnt dlsr(float x, float y, float z, floar r){
    return dls(x,y,z,r,true);
}
Pnt dlsi(float x, float y, float z, floar r){
    return dls(x,y,z,r,false);
}
...
//    Dll nt = Dll(twist.e12*period, twist.e13*period, twist.e23*period, twist.e1ni* pitch, twist.e2ni* pitch, twist.e3ni * pitch);
...
//        Cir c = sp(tc,m);
//        
//        float test = size ( op(dll, gp( c, Pss(-1.0) ) ) );
//        
//        if (test <= 0.0) {
