...
Par surf(const Par a, const Par b, const Par c, const Par d, float u, float v){
}

void main(void) {
    //distances to each control point
    
    vec3 va = upos_a - gl_Vertex;
    vec3 vb = upos_b - gl_Vertex;
    vec3 vc = upos_c - gl_Vertex;
    vec3 vd = upos_d - gl_Vertex;
    float a = va.norm();
    float b = vb.norm();
    float c = vc.norm();
    float d = vd.norm(); 
