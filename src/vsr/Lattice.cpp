   ...
   void  Lattice<T> :: init() {
	...
	void Lattice< Par > :: init(){
		for (int i = 0; i < mNum; ++i){
			...
			mData[i] = Ro::pnt_flat( Ro::null(mGrid[i]), Vec(mGrid[i]).unit(), mSpacing/2.0 );
		}
	}
	...
	void Lattice< Cir > :: init(){
		for (int i = 0; i < mNum; ++i){
			...
			mData[i] =  Ro::pnt_flat( Ro::null(mGrid[i]), b, mSpacing/2.0 );
		}
	}	
...
//	}
	...
	Vec Lattice < T > :: offset(const Pnt& p){
		...
		Vec tp = Vec( v - grid( vxl[0] ) ); //x,y,z distance from lower front corner
		tp[2] *= -1;
		tp /= spacing();
	...
	T Lattice < T > :: eulerPrev ( const Pnt& p) const {
		...
		T tdx;
		for (int i = 0; i < 8; ++i){
			tdx += mPrev[ vxl[i] ] * ( 1.0 / Op::sca( p <= grid ( vxl[i] ) ) );
		}
		return tdx;
	}
...
//	}
	...
	void Lattice < T > :: drawFaces(bool grid){
		for (int i = 0; i < mNumFace; ++i){
			(grid) ? mGrid[mFace[i]].draw() : mData[ mFace[i] ].draw();
		}
	}
	...
	void Lattice < T > :: drawEdges(bool grid){
		for (int i = 0; i < mNumEdge; ++i){
			(grid) ? mGrid[mEdge[i]].draw() : mData[ mEdge[i] ].draw();
		}	
	}
...
//				pdata[ix2] = (neg) ? pdata[ix2-1] * -1 : pdata[ix2-1];//Op::re0( mData[ix2-1], Dlp(0,0,1,od()) ) : mData[ix2-1];		//BACK
				pdata[ix] =  (neg) ?  Op::re0( pdata[ix+1], Dlp(0,0,1,0) ) : pdata[ix+1];
				pdata[ix2] = (neg) ?  Op::re0( pdata[ix2-1], Dlp(0,0,1,0) ) : pdata[ix2-1];
		...
		for ( i = 1; i < mWidth-1; ++i){
			for ( j = 1; j < mDepth-1; ++j ){
				...
				pdata[ix] = neg ? Op::re0( mData[ixo], Dlp(0,1,0,0) ): mData[ixo];//pdata[ixo] * -1 : pdata[ixo]; //
				pdata[ix2] = neg ?  Op::re0( mData[ixo2], Dlp(0,1,0,0) ): mData[ixo2];//pdata[ixo2] * -1 : pdata[ixo2];//			//RIGHT
			}
		}
		//y component
		for ( i = 1; i < mHeight-1; ++i){
			for ( j = 1; j < mDepth-1; ++j ){
				...
				pdata[ix] = neg ?  Op::re0( mData[ixo], Dlp(1,0,0,0) ) : mData[ixo];	//pdata[ixo] * -1 : pdata[ixo];//			//TOP
				pdata[ix2] = neg ? Op::re0( mData[ixo2], Dlp(1,0,0,0) ) : mData[ixo2];//pdata[ixo2] * -1 : pdata[ixo2];//			//BOTTOM
			}
		}
